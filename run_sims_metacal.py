...
from mdetsims.metacal import MetacalSepDetect
from config_metacal import CONFIG
from run_preamble_metacal import get_shear_meas_config
from config_metacal import match, invert_ex
...
def _meas_shear(res):
        return measure_shear_metacal_plus_mof(res, s2n_cut=10, t_ratio_cut=0.5)
...
def gal_match(pos_det, truth_cat):
    ...
    for i in range(len(pos_det)):
        gal_det.append([pos_det[i][0], pos_det[i][1]])
...
def make_catalog(res, truth_cat, pop_index):
    ...
    dic['1p'] = res['1p'][np.where((shear_det == pop_index))]   
    _, shear_det = gal_match(res['1m'], truth_cat)
    dic['1m'] = res['1m'][np.where((shear_det == pop_index))]
    _, shear_det = gal_match(res['2p'], truth_cat)
    dic['2p'] = res['2p'][np.where((shear_det == pop_index))]
    _, shear_det = gal_match(res['2m'], truth_cat)
    dic['2m'] = res['2m'][np.where((shear_det == pop_index))]
...
def _run_sim(seed):
    ...
    try:
        ...
        sim = SIM_CLASS(rng =rng, sel_rng=sel_rng, OneSq_cat=z_red, z_interval=z_inv, shear_population=0, **CONFIG)
        ...
        if match:
            gal = make_catalog(md.result, truth_cat, 0)       
            pres = _meas_shear(gal)
            gal_ex = make_catalog(md.result, truth_cat, 1)
...
if rank == 0:
    print('running metacal', flush=True)
